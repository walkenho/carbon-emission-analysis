...
import numpy as np
import pandas as pd
from emissionsanalysis.data.structure import Ship, AnnualReport, EmissionsData
...
class DataLoader:
    ...
    def _load_df_from_file(filename: pathlib.PosixPath) -> pd.DataFrame:
        ...
        # data cleaning
        df[AnnualReport.co2_per_dwt_distance] = df[AnnualReport.co2_per_dwt_distance].replace(
        ...
        df[AnnualReport.distance_total_imputed_mean] = df[AnnualReport.distance_total].fillna(
            df.groupby(Ship.imo)[AnnualReport.distance_total].transform("mean")
        )

        df[AnnualReport.distance_total_imputed_median] = df[AnnualReport.distance_total].fillna(
            df.groupby(Ship.imo)[AnnualReport.distance_total].transform("median")
        )
    ...
    def load_folder(folder: pathlib.PosixPath) -> EmissionsData:
        ...
        return EmissionsData(
            data=pd.concat([DataLoader._load_df_from_file(filename) for filename in filenames])
        )